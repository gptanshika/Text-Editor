...
main_application.title('TextMate')
...
## reduce size
reduce_size_icon= tk.PhotoImage(file='icons2/open.png')
reduce_size_btn = ttk.Button(tool_bar, image=reduce_size_icon)
reduce_size_btn.grid(row=0, column=9, padx=5)
...
def reduce_size():
    ...
    with open(url1[0]+"compressed"+"."+url1[1],"wb") as myfile:
        myfile.write(zlib.compress(text))
    org_size=os.path.getsize(url) 
    file_size=os.path.getsize(url1[0]+"compressed"+"."+url1[1])
    tk.messagebox.showinfo("File Compressed successfully",f"Original size = {org_size} Bytes\nReduced size = {file_size} Bytes")  

reduce_size_btn.configure(command=reduce_size)
